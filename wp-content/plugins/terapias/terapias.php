...
class configTerapiasMetabox {
	...
	public function add_meta_boxes() {
		foreach ( $this->screen as $single_screen ) {
			add_meta_box(
				'configTerapias',
				__( 'Información de la terapia', 'textdomain' ),
	...
	public function meta_box_callback( $post ) {
		wp_nonce_field( 'configTerapias_data', 'configTerapias_nonce' );
		...
		?><script>
			jQuery(document).ready(function($){
				if ( typeof wp.media !== 'undefined' ) {
					...
					$('.configTerapias-media').click(function(e) {
	...
	public function field_generator( $post ) {
		...
		foreach ( $this->meta_fields as $meta_field ) {
			...
			switch ( $meta_field['type'] ) {
				case 'media':
					$input = sprintf(
						'<input style="width: 80%%" id="%s" name="%s" type="text" value="%s"> <input style="width: 19%%" class="button configTerapias-media" id="%s_button" name="%s_button" type="button" value="Upload" />',
						$meta_field['id'],
						$meta_field['id'],
						$meta_value,
						$meta_field['id'],
	...
	public function save_fields( $post_id ) {
		if ( ! isset( $_POST['configTerapias_nonce'] ) )
			return $post_id;
		$nonce = $_POST['configTerapias_nonce'];
		if ( !wp_verify_nonce( $nonce, 'configTerapias_data' ) )
			return $post_id;
...
if (class_exists('configTerapiasMetabox')) {
	new configTerapiasMetabox;
};
// ./ Campos personalizados
